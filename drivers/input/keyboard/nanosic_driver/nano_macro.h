...
do {                                           \
    if ((int)debuglevel >= (int)__level) {\
        struct timespec64  tsc;\
        struct rtc_time tm;\
        ktime_get_real_ts64(&tsc);\
        rtc_time64_to_tm(tsc.tv_sec,&tm);\
        switch((int)debuglevel) {      \
            case 0: \
            case 1: \
            case 2:  printk("[%02d:%02d:%02d.%03zu] <2>" DRV_TOKEN ": " __format , tm.tm_hour, tm.tm_min, tm.tm_sec, tsc.tv_nsec/1000000, ##__args); break; \
            case 3:  printk("[%02d:%02d:%02d.%03zu] <3>" DRV_TOKEN ": " __format , tm.tm_hour, tm.tm_min, tm.tm_sec, tsc.tv_nsec/1000000, ##__args); break; \
            case 4:  printk("[%02d:%02d:%02d.%03zu] <4>" DRV_TOKEN ": " __format , tm.tm_hour, tm.tm_min, tm.tm_sec, tsc.tv_nsec/1000000, ##__args); break; \
            case 5:  printk("[%02d:%02d:%02d.%03zu] <5>" DRV_TOKEN ": " __format , tm.tm_hour, tm.tm_min, tm.tm_sec, tsc.tv_nsec/1000000, ##__args); break; \
            case 6:  printk("[%02d:%02d:%02d.%03zu] <6>" DRV_TOKEN ": " __format , tm.tm_hour, tm.tm_min, tm.tm_sec, tsc.tv_nsec/1000000, ##__args); break; \
            default: printk("[%02d:%02d:%02d.%03zu] <7>" DRV_TOKEN ": " __format , tm.tm_hour, tm.tm_min, tm.tm_sec, tsc.tv_nsec/1000000, ##__args); break; \
        } \
    }\
} while (0)
...
extern int  Nanosic_sysfs_create(struct device* dev);
...
extern void Nanosic_workQueue_flush(struct nano_worker_client* worker_client);
...
extern int  Nanosic_i2c_read_locked(struct nano_i2c_client* i2c_client,void *buf, size_t len);
extern int  Nanosic_i2c_write_locked(struct nano_i2c_client* i2c_client,void *buf, size_t len);
...
extern bool Nanosic_input_enabled(void);
...
extern void Nanosic_GPIO_irq_release(void);
extern bool Nanosic_GPIO_irq_registered_locked(void);
extern int  Nanosic_GPIO_sleep(bool sleep);
extern int  Nanosic_GPIO_sleep_locked(bool sleep);
extern int  Nanosic_GPIO_irqget(void);
extern int  Nanosic_GPIO_irq_level_locked(void);
extern int  Nanosic_GPIO_reset(void);
extern int  Nanosic_GPIO_reset_locked(void);
extern int  Nanosic_GPIO_set(unsigned int gpio_offset, bool gpio_level);
...
extern int  Nanosic_cache_clear(void);
extern int  Nanosic_cache_release(void);
extern int  Nanosic_cache_put(void);
extern int  Nanosic_PM_init(void);
extern void Nanosic_PM_free(void);
extern int  Nanosic_PM_try_wakeup(void);
extern int  Nanosic_PM_try_wakeup_locked(void);
...
extern struct mutex g_nanosic_lifecycle_lock;
extern struct mutex g_nanosic_hw_lock;
extern bool g_nanosic_hw_ready;
extern bool g_nanosic_hw_recovering;
extern bool g_nanosic_hw_suspended;
...
struct xiaomi_keyboard_data {
    ...
    bool dev_pm_suspend;
    bool main_irq_wake_enabled;
    bool status_irq_wake_enabled;
    int irq;
