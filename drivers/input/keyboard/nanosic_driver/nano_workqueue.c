...
Nanosic_workQueue_schedule(struct nano_worker_client* worker_client)
{
    ...
    if (!work_pending(&worker_client->worker))                  /*当前workqueue是不是在running*/
        ...
        schedule_delayed_work(&worker_client->worker_delay,msecs_to_jiffies(22));
    }
}
...
/** **************************************************************************
 * @func  Nanosic_workQueue_flush
 *
 * @brief Drain queued work without destroying the workqueue
 ** */
void
Nanosic_workQueue_flush(struct nano_worker_client* worker_client)
{
    if(IS_ERR_OR_NULL(worker_client))
        return;
    ...
    if(!IS_ERR_OR_NULL(worker_client->worker_queue))
        flush_workqueue(worker_client->worker_queue);
    atomic_set(&worker_client->schedule_delay_count, 0);
}

/** ************************************************************************//**
 *  @func Nanosic_workQueue_release
 *
...
Nanosic_workQueue_release(struct nano_worker_client* worker_client)
{
	...
	Nanosic_workQueue_flush(worker_client);
